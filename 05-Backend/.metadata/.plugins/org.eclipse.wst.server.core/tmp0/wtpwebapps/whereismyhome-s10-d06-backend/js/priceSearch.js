const projectPath = "http://localhost:8080/whereismyhome-s10-d06-backend/";
...
async function startmap() {
  ...
  console.log(`시 : ${sidoName} 구 : ${gugunName} 동 : ${dongName}`);  
  ...
  let url = projectPath +
    `home?action=search&sido=${sidoName}&gugun=${gugunName}&dong=${dongName}&year=${year}&month=${month}`;
  console.log(url);
  let response = await fetch(url);

  // let response = await fetch(`${url}?${queryParams}`);
  let apts = await response.json();
  // console.log(apts);
  await makeList(apts); //동정보 기반으로
  ...
  searchResult.forEach(async (apt) => {
    ...
    geocoder.addressSearch(apt["address"], function (result, status) {
      ...
      if (status === kakao.maps.services.Status.OK) {
	    ...
	    let content =
          `[${apt["aptName"]}]` + "<br>" + `거래가: ${apt["cost"]}만원`;
...
async function makeList(apts) {
  ...
  apts.forEach((apt) => {
    // console.log(apt);
	  let curDongCode = apt["eubmyundongCode"]
	  
	  let aptObject = {
		cost : parseInt(apt["dealAmount"]) * 1000,
		area : apt["area"],
